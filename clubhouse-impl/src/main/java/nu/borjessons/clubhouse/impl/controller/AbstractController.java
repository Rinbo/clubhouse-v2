...
public abstract class AbstractController {
  ...
  protected <T> T getOptional(Optional<T> option, String identifier) {
    ...
    throw new ResponseStatusException(
        HttpStatus.NOT_FOUND, String.format("That entity could not be found %s", identifier));
  }
  ...
  protected <T> T getOptional(Optional<T> option, Long identifier) {
    if (option.isPresent()) return option.get();
    throw new ResponseStatusException(
        HttpStatus.NOT_FOUND, String.format("That entity could not be found %d", identifier));
  }
